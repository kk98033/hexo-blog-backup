...
#### Input 格式
1. 第一行：一個整數 N，表示樹中的節點數量。當 N 為 0 時，表示輸入結束。

2. 接下來的 N 行：每行代表一個節點的資訊，格式如下：
    第一個數字是節點編號 node（1 到 N 之間的整數）。
    第二個數字是該節點擁有的彈珠數量 marblesCount。
    第三個數字是該節點的子節點數量 childrenCount。
    接下來的 childrenCount 個數字分別是該節點的子節點編號。
