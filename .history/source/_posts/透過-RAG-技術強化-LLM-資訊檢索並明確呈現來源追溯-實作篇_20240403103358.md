...
接下來我會參考這篇 [blog](https://zilliz.com/blog/retrieval-augmented-generation-with-citations) 的文章，建立一個能夠根據文檔資料進行語義搜索的系統。

接下來我們會在 Google Colab 搭建一個基於大型語言模型（LLM）的檢索增強生成模型（RAG）查詢引擎。
